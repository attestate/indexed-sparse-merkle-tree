...
library StateTree {
    ...
    uint256 constant public SIZE = (2**DEPTH) - 1;
    uint256 constant public BUFFER_LENGTH = 1;
     	...
     	for (uint256 d = 0; d < DEPTH; d++) {
        	if ((_index & 1) == 1) {
              if ((_bits & 1) == 1) {
         	    ...
         	    _leaf = keccak256(abi.encode(get(d), _leaf));
              }
        	} else {
              if ((_bits & 1) == 1) {
          		_leaf = keccak256(abi.encode(_leaf, _proofs[d]));
              } else {
          		_leaf = keccak256(abi.encode(_leaf, get(d)));
              }
        	}
